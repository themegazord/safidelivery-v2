...
import { BadgePercent, BarChart3, BookOpen, Bug, Building2, CalendarDays, Clock, CreditCard, FileText, HelpCircle, LogOut, MapPin, QrCode, Star, Truck, Undo2, Users } from "lucide-react";
	...
	const itensMenuSideBar = [
		...
		},
		{
			...
			itens: [
				...
				{ link: '#', icon: <MapPin />, label: 'Configurações de entrega' },
				{ link: '#', icon: <Clock />, label: 'Horários' },
				{ link: '#', icon: <CalendarDays />, label: 'Agendamentos' },
				{ link: '#', icon: <CreditCard />, label: 'Formas de pagamento' },
	...
	return (
		<Sidebar className="bg-primary">
			<SidebarHeader className="flex flex-row items-center gap-2 px-4 py-3">
				...
				<span className="font-semibold text-sm">SAFI Delivery</span>
			</SidebarHeader>
			<SidebarContent>

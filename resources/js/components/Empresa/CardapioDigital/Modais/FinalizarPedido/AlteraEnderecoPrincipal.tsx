...
export default function AlteraEnderecoPrincipal({
    ...
    const enderecoPrincipal =
        auth?.user?.cliente.enderecos.find(
            (endereco) => Boolean(endereco.pivot.principal) === true,
        ) ?? auth?.user?.cliente.enderecos[0];

    const [enderecoAtivo, setEnderecoAtivo] = useState<string>(
        enderecoPrincipal ? String(enderecoPrincipal.id) : "",
    );
    ...
    function atualizaEnderecoPrincipal() {
        axios
            .patch(
                route(
                    "aplicacao.empresa.finalizar-pedido.altera-endereco-principal",
                ),
                {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen} aria-describedby={undefined}>
            <DialogContent className="sm:max-w-sm">
                ...
                <RadioGroup
                    onValueChange={(value) => setEnderecoAtivo(value)}
                    defaultValue={enderecoAtivo}
                >
                    {auth?.user?.cliente.enderecos.map((endereco) => (
                        <FieldLabel
                            htmlFor={String(endereco.id)}
                            key={endereco.id}
                        >
                ...
                <DialogFooter>
                    ...
                    <Button
                        className="cursor-pointer"
                        onClick={() => atualizaEnderecoPrincipal()}
                    >
                        Salvar
                    </Button>
                </DialogFooter>

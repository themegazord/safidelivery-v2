...
import { IGrupoComplemento, IItemPedido } from "@/types/cardapio-digital/item-pedido";
...
function grupoComplementoSaoIguais(
    ...
    const normaliza = (grupos: IGrupoComplemento[]) =>
        [...grupos]
            ...
            .map(g => ({
                ...g,
                complementos: [...g.complementos].sort((a, b) => a.id - b.id)
            }))

    return JSON.stringify(normaliza(grupo1)) === JSON.stringify(normaliza(grupo2))
}
  ...
  function adicionaItemCarrinho(item?: (IItemPedido)) {
    ...
    const itemExistente = carrinho.find(
      (i) => i.id === item.id &&
        grupoComplementoSaoIguais(i.grupo_complemento, item.grupo_complemento) &&
        i.observacao?.trim().toLowerCase() === item.observacao?.trim().toLowerCase()
    );

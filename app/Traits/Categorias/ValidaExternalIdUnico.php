...
trait ValidaExternalIdUnico
{
    ...
    public function validarExternalIdItem(?string $externalId, string $nomeItem, int $cardapioId): ?string
    {
        if (empty($externalId)) {
            return null;
        }
    ...
    public function validarExternalIdItemParaEdicao(?string $externalId, string $nomeItem, int $itemId, int $cardapioId): ?string
    {
        if (empty($externalId)) {
            return null;
        }
        ...
        // Verifica se já existe outro item com este external_id (ignora o próprio item)
        $itemExistente = Item::where('external_id', $externalId)
            ->where('id', '!=', $itemId)
            ->whereHas('categoria', function ($query) use ($cardapioId) {
                $query->where('cardapio_id', $cardapioId);
